---
name: fanout
description: Run independent subtasks in parallel — one git worktree and one implementation sub-agent per task, each opening its own PR — then cross-review every PR. fable never merges; the human does.
---
...
1. Per task, create an isolated worktree:
   `sys_os_shell("git worktree add .worktrees/<task_id> -b cos-team/<task_id>")`.
   Record the worktree path + branch in the registry
   (`.cos-team/registry.json`).
...
5. fable does NOT merge — the PR is the deliverable. When cross-review passes,
...
- Because fable never merges, cross-PR conflicts surface when the human merges,
